...
use crate::{
    ...
    message::{
        InboundMessage, KeyAgreementMessage, Message, MessageDirectionAgreement, NumId, Peer,
        Sender, SeparateParts,
    },
    node::EndpointPair,
    result_early_return,
};
...
pub fn send_error_response<T>(
    send_error: mpsc::error::SendError<T>,
    file: &str,
    line: u32,
) -> String {
...
impl OutboundGateway {
    ...
    pub async fn send(&self, message: &Message, to_be_chunked: bool, key_store: &mut KeyStore) {
        ...
        if dest.endpoint_pair.private_endpoint != EndpointPair::default_socket() {
            self.send_individual(
                Sender::new(dest.endpoint_pair.private_endpoint, dest.id),
                message,
                to_be_chunked,
                key_store,
            )
    ...
    pub async fn send_individual(
        ...
        let sender = if dest.socket.ip().is_private() {
            self.myself.endpoint_pair.private_endpoint
        } else {
        ...
        let chunk_size = if to_be_chunked {
            800 - bincode::serialized_size(&separate_parts).unwrap() as usize
        } else {
            serialized.len()
        };
        ...
        let chunks = chunks
            .enumerate()
            .map(|(i, chunk)| {
                Self::generate_inbound_message_bytes(
                    key_store,
                    dest,
                    chunk,
                    separate_parts.clone(),
                    (i, num_chunks),
                )
            })
    ...
    pub async fn send_agreement(
        ...
        let serialized = result_early_return!(bincode::serialize(&KeyAgreementMessage::new(
            public_key,
            self.myself.id,
            direction
        )));
    ...
    fn generate_inbound_message_bytes(
        ...
        let mut bytes = bincode::serialize(&InboundMessage::new(
            chunk.to_vec(),
            separate_parts.set_position(position),
        ))
        .map_err(|e| e.to_string())?;
        let nonce = key_store
            .transform(dest.id, &mut bytes, Direction::Encode)
            .map_err(|e| e.to_string())?;
