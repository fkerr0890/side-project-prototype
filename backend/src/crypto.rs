...
use crate::{
    ...
    message::NumId,
    message_processing::{ACTIVE_SESSION_TTL_SECONDS, SRP_TTL_SECONDS},
    option_early_return,
};
...
impl KeyStore {
    ...
    pub fn transform<'a>(
        ...
        let (aad, key) = (
            aead::Aad::from("test".as_bytes().to_vec()),
            self.symmetric_keys.get(&peer_id).ok_or(Error::NoKey)?,
        );
        match mode {
            Direction::Encode => {
    ...
    pub fn agree(
        ...
        symmetric_key
            .expand(&[b"sym"], hkdf::HKDF_SHA256)
            .unwrap()
            .fill(&mut symmetric_key_bytes)
            .unwrap();
        let symmetric_key = aead::LessSafeKey::new(
            aead::UnboundKey::new(&aead::AES_256_GCM, &symmetric_key_bytes).unwrap(),
        );
    ...
    pub fn remove_symmetric_key(&mut self, peer_id: &NumId) {
        self.symmetric_keys.remove(peer_id);
    }
    pub fn remove_private_key(&mut self, peer_id: &NumId) {
        self.private_keys.remove(peer_id);
    }
}
