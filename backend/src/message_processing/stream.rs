...
use crate::{http::{self, SerdeHttpResponse}, message::{Heartbeat, Message, NumId, Sender, StreamMessage, StreamMessageInnerKind, StreamMessageKind}, message_processing::ToBeEncrypted, option_early_return, result_early_return, utils::{ArcMap, ArcDeque, ArcSet, ArcCollection, TransientCollection, TtlType}};
...
impl SessionManager {
    ...
    fn start_follow_ups(&self, host_name: String, id: NumId) {
        let (socket, key_store, active_sessions, myself) = (self.outbound_gateway.socket.clone(), self.outbound_gateway.key_store.clone(), self.active_sessions_client.collection().clone(), self.outbound_gateway.myself);
        tokio::spawn(async move {
            loop {
                sleep(Duration::from_secs(2)).await;
                let mut active_sessions = active_sessions.map().lock().unwrap();
                ...
                if !active_session.follow_up(id, |request, dests| {                  
                    for dest in dests.iter() {
                        ...
                        OutboundGateway::send_static(&socket, dest.socket, myself, request, ToBeEncrypted::True(key_store.clone()), true);
                    }
                }) {
                    return
                }
...
struct ActiveSessionInfo {
    ...
    cached_messages: TransientCollection<ArcMap<NumId, (StreamMessage, mpsc::UnboundedSender<StreamResponseType>)>>,
...
impl ActiveSessionInfo {
    ...
    fn follow_up(&mut self, id: NumId, action: impl Fn(&mut StreamMessage, &HashSet<Sender>)) -> bool {
        let mut cached_messages = self.cached_messages.collection().map().lock().unwrap();
        ...
        action(cached_message, &self.active_dests.dests.collection().set().lock().unwrap());
        true
    }
