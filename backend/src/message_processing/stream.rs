...
use crate::{http::{self, SerdeHttpResponse}, message::{Id, Message, StreamMessage, StreamMessageInnerKind, StreamMessageKind}, node::EndpointPair, option_early_return, result_early_return, utils::{TransientMap, TtlType}};
...
impl StreamMessageProcessor
{
            ...
            loop {
                ...
                if !active_session.follow_up(&uuid, |request, dests| {                  
                    for dest in dests.iter() {
                        debug!(uuid = %request.id(), %dest, "Sending follow up");
    ...
    async fn handle_request(&mut self, mut message: StreamMessage) {
        ...
        if dest == EndpointPair::default_socket() {
            ...
            self.start_follow_ups(host_name, uuid);
        }
        else {
    ...
    async fn send_response(&mut self, payload: Vec<u8>, dest: SocketAddrV4, host_name: String, uuid: Id, kind: StreamMessageKind) {
...
impl ActiveSessionInfo {
    ...
    fn handle_cached_message<'a>(uuid: &Id, cached_message: Option<&'a mut (StreamMessage, mpsc::UnboundedSender<StreamResponseType>)>) -> Option<&'a mut StreamMessage> {
        let Some(cached_message) = cached_message else {
            debug!(%uuid, "Prevented request from client, reason: request expired for resource"); return None;
        };
        ...
        debug!(%uuid, "Prevented request from client, reason: already received resource"); None
    }
    ...
    fn follow_up(&mut self, hash: &Id, action: impl Fn(&mut StreamMessage, &HashSet<SocketAddrV4>)) -> bool {
        let mut cached_messages = self.cached_messages.map().lock().unwrap();
        let cached_message = match Self::handle_cached_message(hash, cached_messages.get_mut(hash)) { Some(cached_message) => cached_message, None => { return false; } };
    ...
    fn push_resource(&mut self, message: StreamMessage, set_timer: bool, to_http_handler: mpsc::UnboundedSender<StreamResponseType>) {
        ...
        if cached_messages.contains_key(message.id()) {
            return warn!(uuid = %message.id(), "ActiveSessionInfo: Attempted to insert duplicate request");
        }
    ...
    fn pop_resource(&mut self, message: StreamMessage) {
        ...
        let Some((cached_message, _)) = cached_messages.get_mut(message.id()) else {
            return debug!(senders = ?message.senders(), uuid = %message.id(), "Blocked response from host, reason: unsolicited response for resource")
        };
        *cached_message = message;
        while let Some(uuid) = self.resource_queue.front() {
            ...
            if let StreamMessageKind::Resource(StreamMessageInnerKind::Response) = kind {
                ...
                trace!(uuid = %cached_message.id(), "Popped");
