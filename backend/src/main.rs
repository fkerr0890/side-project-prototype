...
async fn main() {
    ...
    let node = Node::new(private_ip, private_port, &public_ip, Uuid::new_v4().simple().to_string(), None, Some(vec![peer_ip + ":" + &peer_port])).await;
    node.listen(is_start, !is_start, None).await
}