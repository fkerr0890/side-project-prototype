...
impl<K, V> ArcMap<K, V> {
    ...
    pub fn map(&self) -> &Arc<Mutex<FxHashMap<K, V>>> {
        &self.0
    }
}
...
impl<K: Clone> ArcSet<K> {
    pub fn new() -> Self {
        Self(Arc::new(Mutex::new(FxHashSet::default())))
    }
    pub fn set(&self) -> &Arc<Mutex<FxHashSet<K>>> {
        &self.0
    }
}
impl<K: Send + Hash + Eq + Clone + Debug> ArcCollection for ArcSet<K> {
    ...
    fn contains_key(&self, key: &K) -> bool {
        lock!(self.0).contains(key)
    }
    fn push(&mut self, key: K) {
        lock!(self.0).insert(key);
    }
    fn pop(&mut self, key: &K) -> Option<K> {
        lock!(self.0).remove(key);
        None
    }
}
...
impl<K: Send + Hash + Eq + Clone + Debug> ArcCollection for ArcDeque<K> {
    ...
    fn push(&mut self, key: K) {
        lock!(self.0).push_back(key);
    }
    fn pop(&mut self, _key: &K) -> Option<K> {
...
impl TimerOptions {
    ...
    pub fn override_early_return(mut self, override_early_return: bool) -> Self {
        self.override_early_return = override_early_return;
        self
    }
}
...
impl<C: ArcCollection + Clone + Send + 'static> TransientCollection<C> {
    ...
    pub fn insert_key(&mut self, key: C::K, key_label: &str) -> bool {
        self.start_timer(
            key.clone(),
            Some(key),
            None::<fn()>,
    ...
    pub fn set_timer(&mut self, key: C::K, options: TimerOptions, key_label: &str) -> bool {
        self.start_timer(key, None, None::<fn()>, key_label, options)
    }
    pub fn set_timer_with_send_action(
        &mut self,
        key: C::K,
        options: TimerOptions,
        send_action: impl FnOnce() + Send + 'static,
        key_label: &str,
    ) -> bool {
        self.start_timer(key, None, Some(send_action), key_label, options)
    }
    ...
    fn start_timer(
        ...
        key: C::K,
        value: Option<C::K>,
        send_action: Option<impl FnOnce() + Send + 'static>,
        _key_label: &str,
        options: TimerOptions,
    ) -> bool {
        ...
        let (mut collection, ttl, key_clone) = (
...
impl<K: Send + Hash + Eq + Clone + Debug + 'static, V: Send + 'static>
    TransientCollection<ArcMap<K, V>>
{
...
impl<T, U> BidirectionalMpsc<T, U> {
    pub fn channel() -> (Self, BidirectionalMpsc<U, T>) {
        ...
        (
            Self { tx: tx1, rx: rx2 },
            BidirectionalMpsc { tx: tx2, rx: rx1 },
        )
    }
...
macro_rules! time {
    ($block:block, $level:expr) => {{
        ...
        if let Some(level) = $level {
            match level {
                tracing::Level::TRACE => {
                    tracing::trace!("Time elapsed: {:.2?} at {}:{}", duration, file!(), line!())
                }
                tracing::Level::DEBUG => {
                    tracing::debug!("Time elapsed: {:.2?} at {}:{}", duration, file!(), line!())
                }
                tracing::Level::INFO => {
                    tracing::info!("Time elapsed: {:.2?} at {}:{}", duration, file!(), line!())
                }
                tracing::Level::WARN => {
                    tracing::warn!("Time elapsed: {:.2?} at {}:{}", duration, file!(), line!())
                }
                tracing::Level::ERROR => {
                    tracing::error!("Time elapsed: {:.2?} at {}:{}", duration, file!(), line!())
                }
            }
        } else {
